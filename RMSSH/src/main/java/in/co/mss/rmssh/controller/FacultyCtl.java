...
import in.co.mss.rmssh.dto.CollegeDTO;
...
import in.co.mss.rmssh.form.FacultyForm;
import in.co.mss.rmssh.service.CollegeServiceInt;
import in.co.mss.rmssh.service.CourseServiceInt;
import in.co.mss.rmssh.service.FacultyServiceInt;
import in.co.mss.rmssh.validator.FacultyValidator;
...
public class FacultyCtl extends BaseCtl {
	...
	public void preload(Model model) {
		model.addAttribute("collegeList",
				collegeService.search(new CollegeDTO()));
	...
	public String submit(Locale locale, @RequestParam String operation,
		...
		try {
			...
			String msg = messageSource.getMessage("faculty.exists", null,
					locale);
	...
	public String searchList(@ModelAttribute("form") FacultyForm form,
			Model model) {
		int i = service.search(new FacultyDTO()).size();
		int size = 0;
		if (i % 5 == 0) {
		...
		model.addAttribute(
				"list",
				service.search(new FacultyDTO(), form.getPageNo(),
						form.getPageSize()));
		return "FacultyList";
	}
