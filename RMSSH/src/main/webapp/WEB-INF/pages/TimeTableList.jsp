...
<sf:form class="form-inline container  text-center" action="search"
	...
	<div class="form-group">
		...
		<sf:label path="courseId" for="inputPassword3">
			<s:message code="label.courseName1" />
		</sf:label>
		&emsp;
		...
		<c:if test="${!empty list}">
			...
			<c:forEach items="${list}" var="timetable" varStatus="ct">
				<tr>
					...
					<td><c:out value="${timetable.course}" /></td>
					<td><c:out value="${timetable.subject}" /></td>
					<td><fmt:formatDate pattern="dd-MM-yyy" type="date"
							value="${timetable.examinationDate}" /></td>
